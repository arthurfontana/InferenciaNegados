...
|---|---|---|
| `REFERENCIA` | m01 → m02 → m03 (+ m04 backtest opcional) | Gera/recalibra `INF.TABELA_REF_MV` a partir da base histórica com targets |
...
|---|---|---|
| `m00_setup.sas` | E0 | libnames, `options validvarname=v7 mprint`, ODS HTML, valida OBJETIVO/MODO_BASE |
| `m01_montar_base.sas` | E1 | Lê base(s), renomeia, deduplica por `CHAVE`, cruza FPD, deriva flags, pivota SCORE_*/ADICIONAL_*, normaliza canal/score, gera 3 contagens do motor, entrega no grão escolhido |
| `m02_diagnostico.sas` | E2 (Fase 0) | Deriva MIN_N/MIN_EVENTOS (Wilson + power analysis), classifica células, exporta macro vars globais, relatório HTML |
| `m03_tabela_referencia.sas` | E3 (Fase 1) | Agrega por nível hierárquico, fallback, extrapolação exponencial nas caudas, ICs de Wilson, confiabilidade ALTA/MÉDIA/BAIXA/EXTRAPOLADO |
| `m04_aplicar_inferencia.sas` | E4 (Fase 2) | Join hierárquico em cascata, atribui prob_conversao/prob_fpd/fisicos a cada proposta ou célula, backtest |
| `m05_exportar.sas` | E5 | Sumariza por VAR_SEG + DIMS_SAIDA, exporta CSV (delimitador `;`) para o simulador externo |
...
Versão enxuta e autossuficiente com duas macros (`%gerar_inferencia` / `%aplicar_inferencia`) e uma terceira opcional (`%validar_confianca`). Corta o que não é essencial (derivação automática de MIN_N/MIN_EVENTOS, extrapolação exponencial, ICs de Wilson, relatórios HTML) e usa o fallback GLOBAL no lugar da extrapolação. Útil para:
- Explorar/debugar a lógica sem o pipeline completo.
- Rodar `%validar_confianca(ds_base=..., sensibilidade=SIM)` para medir o impacto de alterar MIN_N/MIN_EVENTOS antes de adotar no master.
- Arquivo em ASCII puro (sem acentos) para evitar problemas de encoding.
...
`PESO_FISICO` (default `n_aprovados`) determina sobre quem o físico é calculado. Para reproduzir o CSV do legado `2 - Aplicar` (físico sobre **todas** as propostas, incluindo reprovados), use `PESO_FISICO=n_propostas`.
...
`MARGEM_RELATIVA=0.40`, `ALPHA=0.07`, `PODER=0.75`, `K_EXPONENCIAL=0` (auto-derivado). Resultado: `MIN_N=230`, `MIN_EVENTOS=62`, ~99% das células válidas, cobertura ALTA ~96%. Definidos nos `%let` no topo do `00_MASTER.sas` (ou no bloco de parâmetros do `inferencia_simplificada.sas`). Histórico de calibração em `CONTEXTO.md §5`.

## Armadilhas (SAS e este repo)

- **Encoding:** os `.sas` legados estão em **ISO-8859-1 / Latin-1**, não UTF-8. O `inferencia_simplificada.sas` é ASCII puro de propósito. Ao editar arquivos legados, preserve o encoding original.
...
**Canal PAP** (`CONTEXTO.md §6`): ~52% do volume do PAP cai no grupo MÉDIA (premissa colapsada sem canal), com conversão herdada até 85x acima do real. O parâmetro `CANAIS_EXCLUIR` no master está preparado para tratar isso, mas a decisão de abordagem está pendente. Leia a seção antes de alterar o fallback genérico — o problema é específico do PAP.
